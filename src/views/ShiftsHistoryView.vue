...
import { getGuardListHistory } from '@/apis';
import { stringifyPeriod } from '@/common/helpers/periodHelpers';
import { computed } from 'vue';

const guardListHistory = getGuardListHistory();
const noHistory = computed<boolean>(() => false);
</script>
...
<template>
  <div class="content">
    ...
    <section class="shifts-cards">
      ...
      <el-card v-else v-for="guardPostShifts in guardListHistory" :key="guardPostShifts.guardPostName">
        <template #header>
        ...
        <el-table :data="guardPostShifts.guardList" stripe style="width: 100%">
          <el-table-column prop="guardTime" label="Time" width="220">
            <template #default="{ row }">
              {{ stringifyPeriod(row.guardTime.period) }} ({{ row.guardTime.date.toLocaleDateString('en-GB') }})
            </template>
          </el-table-column>
